...
class PdfParser:
    ...
    def parse_pdf_file(self, filename, processing_funcs, type_discover_func, **kwargs):
        ...
        self.logger.info(json.dumps(processing_funcs, indent=4, default=str))
        extracted_rows = {}
        try:
            ...
            try:
                ...
                self.logger.error(f"Processing failed for file_type: {file_type}, file {filename}, error: {str(err)}")
                import traceback

                self.logger.error(traceback.format_exc())

        except KeyError as ke:
            self.logger.error(f"Missing processing function for file_type: {file_type}, file {filename}, error: {ke}")

        if not extracted_rows:
            self.logger.info("Will try to get data with another parsing method")
            try:
                funcs = [v for _, v in processing_funcs.items()]
