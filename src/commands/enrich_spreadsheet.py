...
class EnrichSpreadsheet(BaseCommand):
    ...
    def check_status_pw(self, case):
        ...
        try:
            ...
            case.case_status = (
                CaseStatus.dismissed if is_dismissed else CaseStatus.active
            )
    ...
    def _process_pdf(self, case, func, file_field_name: str, result_field_name: str):
        try:
            if file_field_name in case.files:
                ...
                self.logger.debug(
                    f"Case {file_field_name}: missing file {file_field_name}"
                )
        except Exception as err:
    ...
    def process_files(self, case: Case):
        ...
        if error_msgs:
            all_msgs = "\n".join(error_msgs)
            self.logger.error(
                f"Case '{case.case_number}': Failed to process case: {all_msgs}"
            )
            case.case_status = CaseStatus.processing_failed

    def _get_dict_formatted(
        self, case: Case, field_name: str, file_field_name: str
    ) -> str:
    ...
    def _check_possible_failure(self, case: Case):
        ...
        ab_data = self._get_dict_formatted(
    ...
    def _prepare_case_data(self, case: Case) -> List[str]:
        ...
        _mapping = {
            ...
            "Creditor Notes": self._get_dict_formatted(
                case, "schedule_d_rows", "url_schedule_d"
            ),
            "Borrower Notes": "",
    ...
    def attorney_csv_parsing(self, case: Case) -> tuple:
        ...
        all_emails = [
            str(f).strip() for f in df["Email"] if self._return_empty_if_nan(f)
        ]
